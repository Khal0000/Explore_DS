...
# y = pd.DataFrame(data, columns=['date', 'Close'])
# y.set_index(['date'],inplace=True)
# # data_inference
# # y
# data_y = pd.concat([data_inference,y], axis=0)
# data_y
...
# fig.add_trace(go.Scatter(x=data_y.index,
#                 y=data_y['Close'], name='forecast'), row=1, col=1) 

# fig.add_trace(go.Scatter(mode ='lines+markers',x=y.index, y=y['pred'], name='forecast', marker=dict(
#     color='LightSkyBlue',
#     size = 120,
...
fig.update_layout(
    title = f'Next candle forecast price = {next_day[0][0]}',
